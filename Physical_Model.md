...
Для решения уравнения эволюции используется метод конечных разностей. Процесс обновления состояния на каждом шаге по времени представлен на схеме:

```mermaid
graph TD
    Input[Вход: Текущее состояние h] --> Deriv[Расчет производных и потоков]
    Deriv --> Update[Обновление толщины:<br/>h = h - dt * FluxGradient]
    Update --> Constraints[Применение ограничений<br/>и граничных условий]
...
2. **Итерационный цикл**: На каждом шаге $\Delta t$ вычисляется расход, его градиент и обновляется толщина.
3. **Граничные условия**: Постоянный приток сверху, свободное вытекание снизу.
...
Добавлена схема алгоритма расчета толщины пленки.