...
class KapitzaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h_mean = self.labels[idx, 1]
        
        # Normalize h readings: (h - h_mean) / h_mean
        # This focuses the model on the wave dynamics rather than the absolute thickness
        h_norm = (h_raw - h_mean) / (h_mean + 1e-7)
        
        # Combine raw and normalized data along a new channel dimension
        # Result shape: [k_max, 2, 100]
        h_combined = torch.stack([h_raw, h_norm], dim=1)
...
class Variant2Model(nn.Module):
    """
    Variant 2 (Improved): CNN Encoder -> Global Average Pooling -> MLP
    Processes each sensor independently and aggregates features across sensors.
    """
    def __init__(self):
        super(Variant2Model, self).__init__()
        
        # Robust 1D-CNN Encoder
        self.encoder = nn.Sequential(
            ...
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.AdaptiveAvgPool1d(1) # Global Average Pooling over time axis
        )
        # Encoder output size: 64
        self.feature_dim = 64
        
        # Total input to MLP: 
        # Aggregated features (64) + global constants (2)
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 2) # Predicts [log10(mu), h_final]
        )

    def forward(self, rho, sigma, k, x, h):
        # h shape: [batch, k_max, 100]
        batch_size = h.shape[0]
        k_max = h.shape[1]
        
        # 1. Process all sensors through the shared encoder
        # Reshape to [batch * k_max, 2, 100]
        h_reshaped = h.view(batch_size * k_max, 2, 100)
        ...
        # 3. Aggregate features across sensors (Average Pooling)
        # We only average active sensors.---
        # Create a mask for active sensors: [batch, k_max, 1]
        mask = (k.unsqueeze(-1).unsqueeze(-1) > torch.arange(k_max).to(DEVICE).view(1, -1, 1))
        #--
        # Simpler version: just use the mean of all k_max. 
        # Since padded values are normalized (h-h_mean)/h_mean = 0, 
        # and the encoder is linear-ish, the mean is a reasonable approximation.
        global_features = torch.mean(encoded_features, dim=1) # [batch, 64]
...
def main():
    ...
    def create_ds(idx):
        return KapitzaDataset(
            rho_norm[idx], sigma_norm[idx], labels[idx], 
    ...
    # Metrics for mu and h_final
    for i, name in enumerate(["Log-Viscosity", "Final Thickness"]):
